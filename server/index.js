 ...
 *
 * Serves the sandbox and answers one endpoint, POST /api/spawn.
 *
 ...
 * Every request returns a spawnable object and a `source` saying where it came
 * from. An error screen at a club fair is an empty booth.
 */
...
import { screen, REDACTED_SPEC, blocklistSize } from './moderation.js';
import { generateSpec, describeError, isConfigured, MODEL } from './claude.js';
...
/** Per-client floor between calls. Stops one kid holding Enter down. */
...
function budgetAvailable(clientId) {
  ...
  // Keep the map from growing forever over a long day.
...
// wifi will die, and a sandbox that cannot load its physics engine is a blank
...
app.post('/api/spawn', async (req, res) => {
  ...
  if (!prompt) {
    return res.json({ spec: normalizeSpec(fromPack('')), source: 'fallback', note: 'empty prompt' });
  }
  ...
  if (screen(prompt).blocked) {
    stats.blocked += 1;
    return res.json({ spec: normalizeSpec(REDACTED_SPEC), source: 'blocked' });
  }

  // 2. No key, mock mode, rate limited, or out of budget: use the pack. The
  //    response shape is identical, so the sandbox cannot tell the difference.
  const budget = budgetAvailable(clientId);
  if (MOCK || !budget.ok) {
    stats.fallbacks += 1;
    return res.json({
      spec: normalizeSpec(fromPack(prompt)),
      source: 'fallback',
  ...
  try {
    ...
    if (screen(normalized.label).blocked) {
      stats.blocked += 1;
      return res.json({ spec: normalizeSpec({ ...normalized, ...REDACTED_SPEC }), source: 'blocked' });
    }

    return res.json({ spec: normalized, source: 'model' });
  } catch (err) {
    stats.errors += 1;
    console.warn(`[spawn] ${describeError(err)} - falling back to pack`);
    return res.json({ spec: normalizeSpec(fromPack(prompt)), source: 'fallback', note: 'api unavailable' });
  }
});

app.listen(PORT, () => {
  console.log(`\n  Physics sandbox running:  http://localhost:${PORT}`);
  console.log(`  Mode:                     ${MOCK ? 'MOCK (offline pack only, no API calls)' : MODEL}`);
  console.log(`  Offline pack:             ${pack.objects.length} objects`);
